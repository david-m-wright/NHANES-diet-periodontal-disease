---
title: "Analysis notes - nutrient intake and periodontal disease"
author: "Project team: David Wright, Jayne Woodside, Anne Nugent, Gerry McKenna"
date: "11 March 2019"
output: html_document
---
```{r checkpoint setup, include = FALSE}
library(checkpoint)
...
```{r prepare modelling dataset, include=FALSE, cache=T}
source(here("Code", "diet_periodontal.R"))
source(here("Code", "helper_functions.R"))
```

A total of 30,468 participants were interviewed across the three waves of interest. Following a previous study of periodontal outcomes using NHANES data (Billings et al), the cohort was restricted to those aged >= 30 years (n = 14,556). Those that did not undergo a medical examination were excluded, leaving a total of 14,071 individuals. Edentate individuals were excluded leaving 11,399 individuals. Those excluded from the periodontal examination for medical reasons were also excluded from the cohort, leaving 11,088 individuals. Individuals in which a periodontal exam was not complete were also excluded, leaving 10,706 individuals. Those with no sites assessed for either pocket depth or clinical attachment loss were excluded, leaving 10,680. Finally, individuals without valid dietary data were excluded leaving a total of `r nrow(nhanes)` individuals in the cohort.
...
```{r list of nutrient variables, echo = F}
kable(diet_names)
```
...
#diet_out <- bind_cols(diet, data.frame(diet_tc_scores))
...
There was variation among TCs showed variation in dispersion and skewness. FOr example, TC1 had a broad right skewed distribution whereas TC6 had a much narrower distribution. Treelet components are not orthogonal (in contrast to PCA) but correlations among TC scores in the nutrient dataset was low. The largest correlation was between TC1 and TC5 with r = `r round(max(cor(diet_tc_scores)[lower.tri(cor(diet_tc_scores), diag = F)]), digits = 2)`.
...
```{r CAL sites assessed, echo = F}
CAL_sites_assessed <- nh %>% 
  count(CAL_sites_assessed) %>%
  mutate(proportion = cumsum(n)/sum(n)*100)

  # ggplot(CAL_sites_assessed, aes(x = CAL_sites_assessed, y = n)) + 
  # geom_bar(stat = "identity") + 
  # theme_light() +
  # labs(x = "Sites assessed", y = "Individuals") +
  # ggtitle("Sites assessed for clinical attachment loss")
  # 
  ggplot(CAL_sites_assessed, aes(x = CAL_sites_assessed, y = proportion)) +
  geom_line() +
...
```{r CAL extent distribution, echo = F}
...
#   mutate(proportion = cumsum(n)/sum(n)*100)
# 
# ggplot(CAL_extent, aes(x = CAL_sites3mm, y = n)) + 
#   geom_bar(stat = "identity") + 
#   theme_light() +
#   labs(x = expression("Sites with CAL">="3mm"), y = "Individuals") +
#   ggtitle("Extent of clinical attachment loss")
# 
# ggplot(CAL_extent, aes(x = CAL_sites3mm, y = proportion)) + 
#   geom_line() + 
#   theme_light() +
#   labs(x = expression("Sites with CAL">="3mm"), y = "Percentage of individuals") +
#   ggtitle("Extent of clinical attachment loss")
...
```
The best model was a variable dispersion model in which all of the adjustment variables were included in the precision sub-model except for `KCAL`. This model had substantially lower BIC than a constant dispersion model (BIC = `r BIC(cal3)` and BIC = `r BIC(cal1)` repectively).
...
Additional models were fitted with polynomial terms for the TCs (cubic splines and quadratic terms) but the most parsimonious model remained the linear fit (BICs of `r BIC(cal4)`, `r BIC(cal5)`, `r BIC(cal6)` for the linear, spline and quadratic fits respecitvely).  
...
```{r second TC score distributions, echo=F}
# Distribution of TC scores
diet_tc_scores2 %>% 
  as_tibble() %>% 
  gather() %>% 
...
As expected, the new TCs were more sparsely loaded, with 17 and 13 non-zero loadings in `TC1` and `TC2` respectively. `TC1` appeared to load heavily on high intake of fats and `TC2` on fibre and minerals. `TC3` was associated with high intake of five fatty acids and `TC4` with vitamins A, B12, retinol and copper. `TC5` loaded on high intake of three fatty acids, `TC6` on high intake of vitamin K and lutein/zeaxantin. `TC7` was associated with high intake of sugar and carbohydrates and `TC8` was associated with high intake of alpha and beta carotene. `TC9` loaded on the same nutrients as `TC2` but represented a contrast between high fibre intake and lower than average intake of the vitamins and minerals in this group.


```{r second treelet loading plots, echo = F, fig.height=11, fig.width=7}
