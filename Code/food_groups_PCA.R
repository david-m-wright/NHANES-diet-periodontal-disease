...
rob5_pca <- Log.lqr(y = nh_grps$prop_CAL_sites3mm, 
                x = model.matrix(update(rob_mod_pca, ~.-SMQ020 + smoking), data = nh_grps_pca),
                p = 0.5)

rob5_coef_pca <- bind_cols(Term = colnames(model.matrix(update(rob_mod_pca, ~.-SMQ020 + smoking), data = nh_grps_pca)),
                       as_tibble(rob5_pca$table, .name_repair = "minimal")) %>% 
  mutate(OR = formatC(exp(Estimate), format = "f", digits = 2)) %>% 
  rename(Sig = V1)


# Format all the PC estimates 
rob5_summary_pca <- nh_grps_pca %>% 
  select(SEQN, matches("^PC[0-9]{1}_dec"),
         CAL_mouth_mean, PD_mouth_mean) %>% 
  cbind(predicted_robust = invlogit(rob5_pca$fitted.values)) %>% 
  ...
  select(PC, PC_decile, `Odds Ratio`, P, `Median sites with CAL`, `Mean CAL (mm)`, `Mean PPD (mm)`) %>% 
...
rob10_pca <- Log.lqr(y = nh_grps$prop_CAL_sites3mm, 
                 x = model.matrix(update(rob_mod_pca, ~.-SMQ020 + smoking + RIDRETH1 + DMDEDUC2), data = nh_grps_pca),
                 p = 0.5)
...
rob10_coef_pca <- bind_cols(Term = colnames(model.matrix(update(rob_mod_pca, ~.-SMQ020 + smoking + RIDRETH1 + DMDEDUC2), data = nh_grps_pca)),
                        as_tibble(rob10_pca$table, .name_repair = "minimal")) %>% 
  mutate(OR = formatC(exp(Estimate), format = "f", digits = 2)) %>% 
  rename(Sig = V1)


# Format all the PC estimates 
rob10_summary_pca <- nh_grps_pca %>% 
  select(SEQN, matches("^PC[0-9]{1}_dec"),
         CAL_mouth_mean, PD_mouth_mean) %>% 
  cbind(predicted_robust = invlogit(rob10_pca$fitted.values)) %>% 
  pivot_longer(cols = matches("^PC[0-9]{1}_dec"), 
               names_to = "PC", 
               values_to = "PC_decile") %>% 
  group_by(PC, PC_decile) %>% 
